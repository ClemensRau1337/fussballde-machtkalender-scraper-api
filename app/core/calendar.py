...
def fetch_calendar_page(plz: str, date_from: str, date_to: str, offset: int, max_results: int,
                        use_cache: bool = USE_CACHE_DEFAULT) -> Dict:
    url = (f"{BASE}/ajax.match.calendar.loadmore/-/datum-bis/{date_to}"
           f"/datum-von/{date_from}/mime-type/JSON/plz/{plz}"
           f"/max/{max_results}/offset/{offset}")
    cache_file = cache_path_for("calendar", f"{plz}_{date_from}_{date_to}_{offset}_{max_results}.json") + ".json"
    ...
    referer = (f"{BASE}/matchkalender/-/plz/{plz}/datum-von/{date_from}"
...
def iter_matches_for_plz(plz: str, date_from: str, date_to: str,
                         page_size: int = 50, sleep_sec: float = SLEEP_SEC,
                         use_cache: bool = USE_CACHE_DEFAULT):
    offset = 0
    last_seen_lastindex = -1
    while True:
        data = fetch_calendar_page(plz, date_from, date_to, offset, page_size, use_cache=use_cache)
...
def collect_matches_for_area(date_from: str, date_to: str, plz_query: str,
                             use_cache: bool = USE_CACHE_DEFAULT) -> List[Dict]:
    plzs = _resolve_plz_inputs(plz_query)
    all_matches: List[Dict] = []
    for plz in plzs:
        for m in iter_matches_for_plz(plz, date_from, date_to, page_size=50, use_cache=use_cache):
